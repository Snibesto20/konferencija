...
import { FaCheck, FaTimes, FaList, FaAlignLeft, FaMapMarkerAlt, FaUserFriends, FaArchive } from "react-icons/fa"
...
export default function EventSettings({target, onClose, onArchive, onUpdate}) {
        ...
        <div className="relative w-200 bg-gray-200 opacity-150 rounded-xs p-4 m-4">
            ...
            <form className="flex flex-col space-y-4 mt-4">
                ...
                <div>
                  ...
                  <Location visible={newEvent.plugins[3] !== undefined} onUpdate={updatePlugin} onRemove={removePlugin} initial={newEvent.plugins[3]} />
                ...
                <div className="flex w-full">
                    <button type="submit" className="flex items-center justify-center gap-2 py-2.5 bg-gradient-to-b from-green-400 to-green-600 w-3/4 border border-black cursor-pointer text-white" onClick={ev => onUpdate(ev, newEvent)}><span className="font-bold ">Išsaugoti</span><FaCheck /></button>
                    <button type="button" className="flex items-center justify-center gap-2 py-2.5 bg-gradient-to-b from-red-400 to-red-600 w-1/4 border border-black cursor-pointer text-white"><span className="font-bold ">Archyvuoti</span><FaArchive /></button>
                </div>
            </form>
        </div>
    )
